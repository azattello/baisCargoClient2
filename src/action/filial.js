...
export const getFilials = async () => {
  try {
    ...
    const response = await axios.get('https://86.107.44.226:3001/api/filial/getFilial');
...
export const fetchFilialByUserPhone = async (userPhone) => {
  try {
      const response = await axios.get('https://86.107.44.226:3001/api/filial/getFilialByUserPhone', {
          params: {
              userPhone
          }
      });
...
export const deleteFilial = async (filialId) => {
  try {
    const response = await axios.delete(`https://86.107.44.226:3001/api/filial/deleteFilial/${filialId}`);
