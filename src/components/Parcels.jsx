...
const Parcels = () => {
    ...
    useEffect(() => {
        const fetchArchive = async () => {
            try {
                const response = await axios.get(`https://86.107.44.226:3001/api/archive/${userId}/getArchive`);
    ...
    const removeBookmark = async (trackNumber) => {
        ...
        if (confirmDelete) {
            try {
                const response = await fetch(`https://86.107.44.226:3001/api/archive/${userId}/delete/${trackNumber}`, {
                    method: 'DELETE',
                });
